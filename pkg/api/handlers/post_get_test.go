	...
	Describe("GET /apis/v1/post/{id}", func() {
		Context("when the post exists", func() {
			It("returns a successful response with post data", func() {
				...
				Expect(postBody["Title"]).To(ContainSubstring("Post title #1"))
				Expect(postBody["Content"]).To(ContainSubstring("Lorem ipsum dolor sit amet..."))
			})
		})
