...
var _ = Describe("CommentPost", func() {
	var (
	...
	BeforeEach(func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(responseBody["message"]).To(ContainSubstring("Comment added successfully"))
	})
	...
	Describe("POST /apis/v1/post/1/comments", func() {
		Context("when the post exists", func() {
			It("returns a successful response", func() {
				w := httptest.NewRecorder()
				r := httptest.NewRequest("DELETE", "/apis/v1/comments/1", nil)
				r.Header.Set("Content-Type", "application/json")
				router.ServeHTTP(w, r)
				...
				err := json.Unmarshal(w.Body.Bytes(), &responseBody)
				Expect(err).ToNot(HaveOccurred())
				Expect(responseBody["message"]).To(ContainSubstring("Comment deleted successfully"))
			})
		})
		Context("when the comment does not exist", func() {
			It("returns a not found response", func() {
				w := httptest.NewRecorder()
				r := httptest.NewRequest("DELETE", "/apis/v1/comments/99", nil)
				r.Header.Set("Content-Type", "application/json")
				router.ServeHTTP(w, r)

				Expect(w.Code).To(Equal(http.StatusNotFound))
				Expect(w.Body.String()).To(ContainSubstring("Comment not found"))
			})
		})
	})
})
